...
void __noreturn elog_errno(const char *str, ...)
{
    va_list ap;
    char *buff;
    ...
    __elog("%s: error is not recoverable, terminating now...\n", program_invocation_short_name);
    free(buff);
    exit(EXIT_FAILURE);
}
...
void error(int error_type, const char *str, ...)
{
    ...
    if (error_type == LOG_NULL) /* special type for elogging */
        __elog("%s: %s", program_invocation_short_name, buff);
    else
        __elog("%s: %s: %s", program_invocation_short_name, type_strings[error_type], buff);
    free(buff);
}
